...
export class DealerDataService {
  ...
  getalluserdata(pageNumber: number, pageSize: number): Observable<dealerVM[]> {
    ...
    return this.httpClient.post<any>(`${environment.apiUrl}Users/dealersearch`, body);
  }

  getNewUsers(dealerEmailId: string): Observable<any> {
    return this.httpClient.get(
      environment.apiUrl + 'Dealer/dealernewusers?emailId=' + dealerEmailId,
    );
  }
  ...
  getSingleBookingDetialByBookingId(id: string): Observable<CarBookingDetailDto> {
    return this.httpClient.get<CarBookingDetailDto>(
      environment.apiUrl + 'BookingUserSlot/GetSingleBookingDetailByBookingId?bookingId=' + id,
    );
  }
  ...
  getBookingDetialsByQrCode(encryptedQrCode: string): Observable<CarBookingDetailDto> {
    return this.httpClient.get<CarBookingDetailDto>(
      environment.apiUrl +
        'BookingUserSlot/GetBookingDetailByEncryptedId?EncryptedId=' +
        encryptedQrCode,
    );
  }

  getPdfOfSingleBooking(bookingId: string): Observable<any> {
    return this.httpClient.get(environment.apiUrl + 'BookingUserSlot/generate-pdf/' + bookingId, {
      responseType: 'blob',
    });
  }
}
